...
| 패턴 | 자동 탈락(무효화) 기준 | 근거 |
|---|---|---|
| 헤드앤숄더 (역헤드앤숄더는 대칭) | ① 종가가 머리를 넘어 회복 → busted 무효 ② 넥라인 돌파 방향으로 5% 초과 진행 → 신호 소진(진입 시점 상실) | Bulkowski(2005) busted 패턴 정의·5% 룰, 되돌림(throwback) ~66%는 넥라인 부근까지 · Lo·Mamaysky·Wang(2000): 패턴 정보력은 완성 직후 집중 |
