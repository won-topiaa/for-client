...
> ⚠️ **이 저장소는 작업용 사본이다. 서버는 여기서 고쳐도 배포되지 않는다.**
> 운영은 `won-topiaa/ma-radar` 의 `main` 브랜치이고 Render 가 그걸 자동
> 배포한다. 이 저장소의 `app/` · `tests/` · `render.yaml` · `requirements.txt`
> 는 2026-08-30 에서 멈춘 **오래된 사본**이라, 그 뒤의 운영 장애 수정
> (DB IPv4 고정 · NullPool · DB 없이도 부팅 · 분석 결과 캐시)이 전혀 없다.
> 여기에 서버를 고쳐 커밋하면 "배포했다"고 착각한 채 서비스는 그대로다 —
> 2026-08 에 실제로 그럴 뻔했다.
>
> 반대로 **미니앱(`apps-in-toss/`) 은 이 저장소가 최신이다.** 앱은 여기서
> 고치고 `npx ait build` 로 `.ait` 를 만들어 콘솔에 올린다.
>
