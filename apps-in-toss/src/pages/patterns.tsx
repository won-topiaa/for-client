...
const PATTERNS: {
  ...
  },
  {
    ...
    label: '역헤드앤숄더',
    emoji: '🔺',
    plain: '헤드앤숄더를 뒤집은 모양이에요. 바닥에서 저점이 점점 높아질 때 나옵니다.',
    detail:
      '저점 세 개 중 가운데가 가장 깊고, 그 뒤로 저점이 높아집니다. 파는 힘이 ' +
      '잦아드는 과정으로 보는 고전적인 바닥 전환형이에요. 되돌림 고점 둘을 이은 ' +
      '선(넥라인)을 위로 넘어서면 완성으로 보지만, 넘지 못하고 되돌아가는 경우도 ' +
      '흔합니다.',
  },
  {
    key: 'head_shoulders',
    label: '헤드앤숄더',
    emoji: '🔻',
    plain: '봉우리 셋 중 가운데가 가장 높은 모양이에요. 오르던 흐름이 힘을 잃을 때 나옵니다.',
    detail:
      '왼쪽 어깨 → 머리 → 오른쪽 어깨 순으로 고점 세 개가 만들어지고, 두 번의 ' +
      '되돌림 저점을 이은 선(넥라인)이 기준이 됩니다. 머리 뒤의 고점이 낮아진다는 ' +
      '것은 사는 힘이 줄었다는 뜻으로 읽어요. 다른 넷과 달리 이것만 하락 쪽을 보는 ' +
      '패턴이라, 같은 화면에 있다고 같은 뜻으로 읽지 않도록 주의하세요.',
  },
  {
    key: 'cup_handle',
    label: '컵앤핸들',
