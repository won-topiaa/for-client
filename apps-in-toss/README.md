...
> ⚠️ **저장소가 두 개다.** 미니앱 소스는 **이쪽(`won-topiaa/for-client`)이
> 최신**이라 `.ait` 빌드는 여기서 한다. 반면 **서버·웹사이트는
> `won-topiaa/ma-radar` 의 `main` 이 운영**이고 Render 가 그것만 배포한다 —
> `app/` 이나 `static/` 을 이 저장소에서 고쳐도 서비스는 바뀌지 않는다.
> 자세한 내용: `ma-radar` 저장소의 `docs/REPOS.md`
