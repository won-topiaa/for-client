...
- **무료 시세 데이터** 기본 사용 — API 키·IP 등록 불필요 (국내: FinanceDataReader/네이버 · 미국: yfinance→FDR→Stooq 3중 폴백)
...
미국은 yfinance(야후)를 먼저 쓰고 FDR→Stooq 순으로 폴백합니다.
...
| 값 | 설명 |
|---|---|
| `auto`(기본)/`free` | 무료 시세 (국내 FDR/네이버 · 미국 yfinance→FDR→Stooq). 키·IP 불필요, 공개 배포 가능 |
...
직전 20봉 평균이 선 위(지지로 접근하는 문맥)여야 하고, 지지 성공 2회 이상인
선만 인정. 정렬은 선의 신뢰 점수(Wilson 하한 × log(1+가중 성공)) 순.
